...
" (define reduce (lambda (binop zero xs) (if (pair? xs) (if (pair? (cdr xs)) (binop (car xs) (reduce binop zero (cdr xs))) (car xs)) zero)))"
" (define foldl (lambda (binop zero xs) (if (pair? xs) (foldl binop (binop zero (car xs)) (cdr xs)) zero)))"
" (define foldr (lambda (binop zero xs) (if (pair? xs) (binop (car xs) (foldr binop zero (cdr xs))) zero)))"
...
" (define and (macro args (if (pair? args) (if (pair? (cdr args))"
"   (list let (list (list '_test_ (car args))) (list if '_test_ (cons 'and (cdr args)) '_test_)) (car args)) #t)))"
" (define or (macro args (if (pair? args) (if (pair? (cdr args))"
"   (list let (list (list '_test_ (car args))) (list if '_test_ '_test_ (cons 'or (cdr args)))) (car args)) #f)))"
